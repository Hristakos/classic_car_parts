...
def upload_listing_image(file_path,post_date)
    uploads = {}
...
def create_listing(headline, description, user_id, suburb, post_date,price, img_path)
    img_url = upload_listing_image(img_path, post_date)
...
def all_listings_order_by_lowest_price()
    listings = run_sql("select * from listings order by price asc",[])
    listings
end
